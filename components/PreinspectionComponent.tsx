...
const Preinspection: React.FC<Props> = ({ route, navigation }) => {
  ...
  const tBoxesStatusProcessed = item1.filter((item) => item.statusId === 1)
    .length;
  const tBoxesStatusOutline = item1.filter((item) => item.statusId === 2)
    .length;
  const tBoxesMissing = item1.filter((item) => item.statusId === 0).length;
  const tHousesToOutline = item2.filter((item) => item.toOutline).length;
  const tHousesInOutline = item2.filter((item) => item.statusId === 2).length;
